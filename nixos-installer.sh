...
parted -s -a optimal ${HDD} mkpart primary efi fat32 ${EFI_PART_START} ${EFI_PART_END}
...
parted -s -a optimal ${HDD} mkpart primary boot ext4 ${BOOT_PART_START} ${BOOT_PART_END}
check_error("Failed to create Boot (/boot) partition.")

# Create the encrypted partition.
parted -s -a optimal ${HDD} mkpart primary ${LUKS_PART_NAME} ${LUKS_PART_START} ${LUKS_PART_END}
check_error("Failed to create encrypted partition.")

# Mount the encrypted partition.
cryptsetup luksFormat /dev/sda3 ${LUKS_KEY}
check_error("Failed to format encrypted partition.")

cryptsetup luksOpen /dev/sda3 ${LUKS_VOL_NAME} --key-file ${LUKS_KEY}
